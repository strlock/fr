...
var EnvatoWizard = (function ($) {
    ...
    function window_loaded() {
        $('#plugins-area .thmv-switch [type="checkbox"]').on('click', function (e) {
            e.preventDefault();
    ...
    function PluginManager() {
        ...
        function ajax_callback(response) {
            ...
            if (typeof response == 'object' && typeof response.message != 'undefined') {
                ...
                if (typeof response.url != 'undefined') {
                    ...
                    $.post(response.url, response, function (response) {
                        $status_node.text(response.message);
                        complete();
                    }).fail(ajax_callback);
                    ...
                    is_success = (typeof response.success !== 'undefined') ? response.success : false;
                    //set the full file path from the newly installed plugin
                    if (response.plugin_file !== 'undefined') {
                        $current_node.attr('data-plugin', response.plugin_file);
                    }
                    if (response.deactivate_url !== 'undefined') {
                        $current_node.attr('data-deactivate_url', response.deactivate_url);
                    }
                    
                    var error_message = (typeof response.data !== 'undefined' && typeof response.data.errorMessage !== 'undefined') ? response.data.errorMessage: response.message;
                    $status_node.text((typeof error_message !== 'undefined') ? error_message : 'Some error occurrred.');
                    complete();
                }
        ...
        function process() {
            if (current_item) {
                console.log(action);
                if (action == plugins_params.activate_action) {
                    //if going for activation, and don't have activate data, addd it
...
jQuery(document).ready(function ($) {
    ...
    $('#misc_settings input').on('change', function (event) {
