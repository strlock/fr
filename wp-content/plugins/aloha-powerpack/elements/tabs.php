...
class Themo_Widget_Tabs extends Widget_Base {
    ...
    public function get_style_depends(){
        ...
        wp_register_style( $this->get_name(), THEMO_URL . 'css/tabs.css', array(), $modified2 );
        return [$this->get_name()];
    }
    public function get_script_depends(){
        $modified = filemtime(THEMO_PATH.'js/thmv-tabs.js');
        wp_register_script($this->get_name(), THEMO_URL . 'js/thmv-tabs.js', ['jquery','elementor-frontend'], $modified, true);
    ...
    public function get_title() {
        return __('Themovation Tabs', ALOHA_DOMAIN);
    }
    ...
    protected function register_controls() {
        ...
        $repeater->add_control(
                'tab_title_size',
                [
                    'label' => __('Tab Title HTML Tag', ALOHA_DOMAIN),
        ...
        for ($i = 0; $i < $this->totalTabChoices; $i++) {
            ...
            if ($i == 0) {
                $array['default'] = __('Tab Content', ALOHA_DOMAIN);
            } else {
                unset($array['default']);
            }
            $repeater->add_control(
                ...
                foreach ($tabs as $index => $item) :
                    ...
                    <div <?php echo $this->get_render_attribute_string($tab_content_setting_key); ?>>
                        ...
                        foreach ($allFields as $tabContent) {
                            $title = isset($tabContent['title']) && !empty($tabContent['title']) ? $tabContent['title'] : false;
                            $price = isset($tabContent['price']) && !empty($tabContent['price']) ? $tabContent['price'] : false;
                            $content = isset($tabContent['content']) && !empty($tabContent['content']) ? $tabContent['content'] : false;
                            ?>
                            <div <?php echo $this->get_render_attribute_string($columns_attribute); ?> <?php echo empty($content) ? 'data-no-text="1"' : '' ?>>
                                ...
                                <?php if (!empty($title) || !empty($price)) : ?>
                                    <div class="thmv-title-price-block">
                                        <?php if (!empty($title)): ?>
                                            <<?php echo esc_attr($item['tab_title_size']); ?> class="thmv-tab-heading"><?php echo esc_html($title) ?></<?php echo esc_attr($item['tab_title_size']); ?>>
                                        <?php endif; ?>
                                        <?php if (!empty($price)): ?>
