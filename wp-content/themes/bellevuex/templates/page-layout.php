...
$outer_container_open = empty($outer_container_open) ? '': $outer_container_open;//avoid null so to avoid a warning
$outer_row_open = themo_return_outer_tag("<div class='row'>",$has_sidebar);
$outer_row_open = empty($outer_row_open) ? '': $outer_row_open;//avoid null so to avoid a warning

$outer_container_close = themo_return_outer_tag("</div><!-- /.container -->",$has_sidebar);
$outer_container_close = empty($outer_container_close) ? '': $outer_container_close;//avoid null so to avoid a warning
$outer_row_close = themo_return_outer_tag("</div><!-- /.row -->",$has_sidebar);
$outer_row_close = empty($outer_row_close) ? '': $outer_row_close;//avoid null so to avoid a warning
...
$inner_container_open = empty($inner_container_open) ? '': $inner_container_open;//avoid null so to avoid a warning
$inner_row_open = themo_return_inner_tag("<div class='row'>",$has_sidebar);
$inner_row_open = empty($inner_row_open) ? '': $inner_row_open;//avoid null so to avoid a warning

$inner_container_close = themo_return_inner_tag("</div><!-- /.container -->",$has_sidebar);
$inner_container_close = empty($inner_container_close) ? '': $inner_container_close;//avoid null so to avoid a warning
$inner_row_close = themo_return_inner_tag("</div><!-- /.row -->",$has_sidebar);
$inner_row_close = empty($inner_row_close) ? '': $inner_row_close;//avoid null so to avoid a warning
...
$main_class_open = empty($main_class_open) ? '': $main_class_open;//avoid null so to avoid a warning
$main_class_close = themo_return_outer_tag('</div><!-- /.main -->',$has_sidebar);
$main_class_close = empty($main_class_close) ? '': $main_class_close;//avoid null so to avoid a warning
