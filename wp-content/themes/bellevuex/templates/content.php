...
<div class="post-inner">
    <?php if ('-masonry' == $masonry_template_key) { ?>
    <?php get_template_part('templates/entry-meta'.$masonry_template_key); ?>
    <?php } ?>
    <?php
    if ( (isset($args['thmv_blog_widget']) && $args['thmv_blog_widget']) || !is_single()) {
        $th_post_title = the_title('','',false);
        if(!$th_post_title > ""){
            $th_post_title = esc_html__( '(no title)', 'bellevue' );
        }
        ?>
        <h3 class="post-title"><a href="<?php the_permalink(); ?>"><?php echo wp_kses_post( $th_post_title ); ?></a></h3>
    <?php } ?>
